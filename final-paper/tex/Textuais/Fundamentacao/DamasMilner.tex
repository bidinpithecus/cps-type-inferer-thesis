...
A introdução do sistema Damas-Milner trouxe duas contribuições principais: a definição de um sistema de tipos robusto e a criação de um algoritmo, o Algoritmo W, capaz de inferir o tipo mais geral (também chamado de \textit{principal type-scheme}), conforme demonstrado em~\citeonline{damas1984assignment}.
...
O algoritmo de unificação, \Unify, descrito na Figura~\ref{algo:unify}, gera a unificação mais geral possível.
...
O algoritmo \Unify\ utiliza a função de verificação de
...
que resultaria em inconsistência no sistema de tipos~\cite{ribeiro2016mechanized}.
