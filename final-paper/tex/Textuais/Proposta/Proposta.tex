...
\citeonline{thielecke1997categorical} propôs um sistema de tipos monomórfico para CPS que define um único construtor (chamado de negação poliádica) para representar continuações \cite{torrens2024operational}:
...
Portanto $\{\text{x:}\tau \text{, k:} \neg \tau\} \vdash \text{k}\langle x \rangle$ estabelece que em um contexto em que \texttt{x} possui tipo $\tau$, \texttt{k} possui tipo $\neg \tau$, a expressão $\text{k}\langle \vec{x} \rangle$ não irá falhar~\cite{thielecke1997categorical}.
\citeonline{torrens2024operational} demonstram a segurança (\textit{type safety}) desse sistema, provando preservação e progresso, ou seja, qualquer expressão bem tipada não apresentará comportamento inesperado quando reduzida.
